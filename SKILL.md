...
2. ファイルごとに種別判定し、方針を分ける。
   ...
   - **生成されるロックファイル**（`yarn.lock`, `package-lock.json`, `pnpm-lock.yaml` など）：手編集しない。対応する `package.json` 側のコンフリクトを先に解決したうえで、`yarn install` / `npm install` / `pnpm install` のいずれか（プロジェクトの `packageManager` フィールドやロックファイルの種類から判定）を実行し、自動再生成に任せる。
