...
2. ファイルごとに種別判定し、方針を分ける。
   ...
   - **生成物ファイル**（`yarn.lock` / `package-lock.json` / `pnpm-lock.yaml` などのロックファイルに限らず、ビルド成果物・自動生成コードなど「人が直接編集しない」ファイル全般）：**手編集で解決することは禁止**。
...
- 生成物ファイル（ロックファイル等）の手編集による解決。
- 生成物ファイルの再生成方法が不明な場合に、確認せず推測で解決すること。
...
- 生成物ファイルの再生成コマンド実行によって追加で `git add` したファイル・差分があれば、それも報告に含める（何を・なぜ追加で add したか）。
